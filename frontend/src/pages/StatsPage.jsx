      ...
      <div className="flex-1 lg:ml-60 min-w-0">
        ...
        <div className="p-4 md:p-6 max-w-xl mx-auto pb-36">
          <div className="mb-5 mt-2">
            <h2 className="text-xl font-black text-white">Activity Insights <span className="text-yellow-400">✦</span></h2>
          ...
          <div className="bg-yellow-400 rounded-3xl p-5 mb-4">
            ...
            <div className="w-full rounded-full h-3 bg-yellow-200">
              <div className="h-3 rounded-full transition-all duration-700 bg-black" style={{ width: `${percent}%` }} />
            </div>
              ...
              {last7Days.map((d, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex flex-col items-center justify-end" style={{ height: "80px" }}>
                    <div className="w-full rounded-t-lg transition-all duration-500 bg-yellow-400" style={{ height: `${(d.done / maxVal) * 80}px`, minHeight: d.done > 0 ? "4px" : "0" }} />
                    <div className="w-full rounded-t-lg bg-gray-800" style={{ height: `${((d.total - d.done) / maxVal) * 80}px`, minHeight: (d.total - d.done) > 0 ? "4px" : "0" }} />
                  </div>
              ...
              <div className="flex flex-col gap-2">
                {tasks.slice(0, 5).map((task) => (
                  <div key={task.id} className="flex items-center gap-3 p-3 bg-gray-900 rounded-2xl">
                    ...
                    <p className={`text-sm flex-1 ${task.is_completed ? "line-through text-gray-600" : "text-gray-300"}`}>{task.title}</p>
